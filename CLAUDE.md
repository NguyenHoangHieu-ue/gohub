...
- [ ] **GA4 App connect** — Hiếu cấp quyền Viewer cho service account
  `ais-gemini-key-88b236e5f62d4cf@612144486106.iam.gserviceaccount.com` trên property App `465150028`
  (`analytics.google.com` → Admin → chọn property App → Property Access Management → + → paste email →
  Viewer). Xong báo lại để thêm entry `gohub-app` vào Supabase `app_settings.ga4_configs` + kiểm tra
  code tab Website (`platform=app` hiện lọc theo `platform ios/android` TRÊN CÙNG property web đang chọn —
  nếu App thật ra là property riêng thì cần sửa để trỏ đúng site, không chỉ đổi filter).
