...
| tsc + `next build` | PASS |
| ✅ **s189 (2026-09-05, tiếp) — Phase 5 xong: tách nốt B2BTierSection** | Tách `B2BTierSection` (~1080 dòng, phần lớn nhất còn lại) khỏi `quarterly/page.tsx` sang `components/quarterly/b2b-tier-section.tsx` — gồm cả TIER_COLORS/REGION_META, CostLine/CustomerCostEdits, parseCostLines/costLabel/cloneCostLines/lineTotal/mLabel (verify grep xác nhận không leak sang phần còn lại trang). `quarterly/page.tsx`: 2688→1564 dòng (tổng từ gốc 3077 dòng: **-49%**). tsc + vitest 177/177 PASS + `next build` "Compiled successfully". **Phase 5 (dọn 4 file FE khổng lồ) coi như XONG tách cơ học**: quarterly 3077→1564, channels 1944→1843, to-gau 2843→1224, my-metrics 1809→629. **Cần Hiếu**: QA `B2BTierSection` trên staging (tab B2B trong Quarter Report — bảng tier, expand KH, sửa/import/export CH.Cost, target KH) cùng lúc với to-gau/My Metrics còn nợ ở mục dưới. |
| ⏳ **s189 (2026-09-05) — Phase 5 plan rebuild: tách cơ học to-gau + my-metrics** | Tiếp s188. QA Chrome thật trên staging (Creator, data thật) xác nhận s188 (Quarter Report Tổng quan: KpiCard/ColInfo tooltip/MomBadge/MonthSubRow toggle/QtSummaryRow+QtTargetRow/PivotTable B2C; Channels: B2BCustomerDetail) đều đúng, không bug do refactor — coi PASS QA. Làm tiếp 2 file còn lại theo đúng nguyên tắc "chỉ tách cơ học, không gộp JSX khác nhau": `to-gau/[id]/page.tsx` 2843→1224 dòng (-57%, tách Avatar/SettingsModal/DocsPanel/NotesPanel/WikiPanel/FilePreviewItem/AttachmentDisplay/ConfirmModal+useConfirm sang `components/to-gau/*.tsx`, types sang `lib/to-gau-types.ts`, format helpers sang `lib/to-gau-format.tsx`); `my-metrics/page.tsx` 1809→629 dòng (-65%, tách LarkReviewPanel/EvidenceCard/DatapoolDetailTable/SkuScanSection/BegauInsightsSection/ScanResultBox/LarkConfigModal + UI dùng chung ProgressBar/SourceBox/NotesDrawer/DataTable sang `components/my-metrics/*.tsx`, types+hằng số sang `lib/my-metrics-types.ts`, format helpers sang `lib/my-metrics-format.ts`; 2 chart dynamic-import SkuMoversChart/TopUsersChart dời theo component, đổi sang alias path `@/app/(dashboard)/...`). tsc + vitest 177/177 PASS + `next build` "Compiled successfully" sau mỗi file (lỗi collect-page-data sau đó do thiếu `.env.local`, pre-existing, không liên quan). **CHƯA làm**: `B2BTierSection` (~1080 dòng, còn lại trong `quarterly/page.tsx`, quá lớn cho 1 pass — để dành riêng). Plan: `C:\Users\nhhie\.claude\plans\effervescent-zooming-wilkinson.md`. **Cần Hiếu**: QA to-gau (Chat/Tài liệu/Wiki/Settings modal) + My Metrics trên staging trước khi coi Phase 5 xong hoàn toàn. |
| ✅ **s188 (2026-09-04) — Phase 5 plan rebuild: tách cơ học quarterly + channels** | Đã QA xong ở s189 (xem trên) — PASS, không bug. `quarterly/page.tsx` 3077→2688 dòng (tách KpiCard/TableHead/ColInfo/MomBadge/MonthSubRow/QtSummaryRow/QtTargetRow/PivotTable sang `components/quarterly/*` + types/format helpers sang `lib/quarterly-types.ts`/`lib/quarterly-format.ts`); `channels/page.tsx` 1944→1843 dòng (tách B2BCustomerDetail sang `components/channels/`). |
| ✅ **s187 (2026-09-04) — Phase 4 plan rebuild: rate-limit chuyển Upstash Redis + mở rộng 4 route** | `lib/rate-limit.ts` đổi từ in-memory Map (chỉ đúng 1 Vercel instance, nhiều instance chạy song song đếm riêng — không chặn đúng tổng request) sang Upstash Redis khi có env `UPSTASH_REDIS_REST_URL`/`_TOKEN` (đếm dùng chung mọi instance) — thiếu env thì tự rơi về in-memory cũ, không vỡ gì (opt-in). `checkRateLimit()` đổi thành async, cập nhật 2 chỗ gọi cũ (`/api/chat`, `/api/creator-ai/chat`) thêm `await`. Mở rộng thêm `POST /api/analytics/query` (60/min/user) + `POST /api/admin/sql-query` (30/min/user, bảo vệ DB pool 3-connection) đúng khuyến nghị `operations-runbook.md` §7 viết từ trước nhưng chưa ai làm. Thêm `rate-limit.test.ts` (4 test, trước đây 0 test cho module dùng từ s159). tsc + vitest 177/177 PASS. **Cần Hiếu**: tạo tài khoản Upstash (free tier) + set 2 env trên Vercel Production+Preview để limiter chặn thật cross-instance — chưa set thì vẫn chạy đúng như trước (chỉ chưa có lợi ích cross-instance). Plan: `C:\Users\nhhie\.claude\plans\effervescent-zooming-wilkinson.md`. Phần "cache tag hoá diện rộng" trong plan gốc — audit lại thấy `analytics-helpers.ts` (`flushB2BCostCaches`/`B2B_COST_CACHE_PREFIXES`, đã fix s169(c)) đã đủ tốt, KHÔNG tìm thấy vấn đề cụ thể mới để sửa → không làm thêm, tránh việc làm không có giá trị thật. |
| ✅ **s186 (2026-09-04) — Phase 3 plan rebuild: dọn tàn dư Orders/Order Report đã gộp trước đó** | Audit phát hiện: Orders + Order Report ĐÃ được gộp từ 1 session trước (`/analytics/order-report/page.tsx` chỉ còn 5 dòng redirect, Orders thật ra gọi `/api/analytics/order-report`) — nhưng chưa dọn sạch: 2 route API chết (`GET /api/orders` cũ, `GET /api/staff-list`) + 2 file wiki mô tả sai kiến trúc (1 file nói API là `/api/orders`, 1 file mô tả hẳn 1 trang đã redirect). Verify kỹ bằng grep toàn `web/src` trước khi xoá — suýt xoá nhầm `/api/orders/export` (đang được **Products BI** dùng, khác `/api/orders`) và `/api/order-sources` (dropdown filter Source của chính Orders, vẫn sống) nếu không kiểm tra từng route riêng. Gộp 2 wiki thành 1 (`analytics-orders.md`), xoá `analytics-order-report.md`, cập nhật `HOME.md`. nav.ts/sidebar/analytics-roles.ts đã đúng từ trước, không cần sửa. tsc + vitest 173/173 PASS. Plan: `C:\Users\nhhie\.claude\plans\effervescent-zooming-wilkinson.md`. Đã hỏi Hiếu trước khi làm (chỉ merge Orders/Order Report, KHÔNG bỏ modal "Báo cáo Quý" Dashboard — Hiếu chưa chốt việc đó). Candidate gộp khác (CS Troubleshoot/Feedback/3HK Usage/Website/All-Time) cần số usage thật `app_usage_events` trước khi quyết — Hiếu chọn bỏ qua bước này, chưa đụng. |
| ✅ **s185 (2026-09-04) — Phase 2 plan rebuild: fix bug thật All-Time + timezone date-math + migrate BOD/Quarter/B2B family sang engine** | Bug thật đã fix: (1) `all-time-performance/route.ts` — query chính CASE tham chiếu `c.name`/`c.price_list_name` nhưng thiếu `LEFT JOIN dim_customer c`, với `DEFAULT_TIER_KEYWORDS` mặc định (không rỗng) → CHẮC CHẮN lỗi SQL thật, không chỉ "có thể sai" như wiki ghi trước; (2) `bod-data.ts` + `cron/refresh-monthly-kpis` — `getDaysInMonth`/`daysInMonth` cục bộ parse `new Date(...)` UTC rồi đọc lại `.getFullYear()/.getMonth()` LOCAL, lệch ngày trên máy timezone offset âm (đúng trên Vercel/UTC nên chưa lộ production). Thêm `lib/analytics-engine/date-math.ts` (thuần, zero-dep, client-safe, không phụ thuộc timezone runtime) làm nguồn duy nhất, thay 2 bản trùng. Migrate `squad-progress`/`quarterly-report`/`quarterly-b2b-customers` (BE) + `b2b/page.tsx` (FE, đóng 1 TODO "OOP refactor" để sẵn từ trước) sang gọi hàm chung thay viết tay lại — công thức verify tương đương tuyệt đối trước khi đổi. Channels + B2B server routes đã audit, sạch, không cần sửa. tsc + vitest 173/173 PASS (thêm `date-math.test.ts` — 8 test mới, gồm test timezone-invariance). `next build` không verify đủ trên máy này (thiếu `.env.local`, pre-existing, không phải do đổi lần này). Plan: `C:\Users\nhhie\.claude\plans\effervescent-zooming-wilkinson.md`. Còn lại trong Phase 2 theo plan: Dashboard `monthly-kpis` route đã audit sạch (route sống, không cron) — coi Phase 2 xong cho nhóm route cost/projection; Phase 3+ (gộp tab trùng, Upstash, dọn FE) chưa làm. |
| ✅ **s184 (2026-09-04) — Phase 1 plan rebuild: engine `getKpiFactor`/`getElapsedRatio` + 30 test mới** | `analytics-engine/quarter-projection.ts` thêm 2 hàm dùng chung thay cho `kpiPrFactor`/`monthKpiFactor` (quarterly/page.tsx) và `kpiFactorOf`/`elapsedRatioOf` (squad-progress/route.ts) tự định nghĩa lại y hệt nhau — root cause bug s166/s181/s182. **CHƯA migrate route/page nào dùng hàm mới** (đúng scope Phase 1: build+test engine, route đổi ở Phase 2). Thêm 30 test case (cost-engine/projection/quarter-projection/b2b-customer-cost + excludeInactiveCustomers/buildIsStrategicSql) — trước đây 0 test cho các module tính CM1/chi phí dù đây là nơi gây phần lớn bug lặp lại. **Điều chỉnh so với plan gốc**: audit code thật cho thấy claim "20 file tự chép lại calcChCost" trong plan hơi quá — đa số route ĐÃ import đúng `calcChCostForPeriod`/`fetchCustomerCosts` dùng chung, chỉ nhóm Quarter Report (squad-progress + quarterly/page.tsx) thật sự có 3 bản định nghĩa tay trùng nhau, đã fix đúng chỗ đó. Không tạo `filters.ts`/`cache.ts` mới như plan gốc đề — `analytics-helpers.ts` đã gộp tốt rồi, tạo thêm file song song chỉ phân mảnh thêm. Phát hiện phụ (không sửa, ghi lại): `buildQuarterMonthMeta` trộn `new Date(isoString)` (UTC) với `new Date(y,m,d)` (local) — đúng trên Vercel (UTC) nhưng lệch ±1 ngày nếu chạy máy dev timezone khác. tsc + vitest 165/165 PASS. Plan đầy đủ: `C:\Users\nhhie\.claude\plans\effervescent-zooming-wilkinson.md`. |
...
thread — trước hẹp (chỉ "tạo/onboard sản phẩm mới" + "hỏi gói nước nào") — nay `SYSTEM_PROMPT`
(`lib/okr-lark-classify.ts`) nêu rõ 3 nhóm ví dụ SLA: tạo/thêm/add sản phẩm · cung cấp/kiểm tra/xác nhận
thông tin · báo lỗi sản phẩm, cộng "các dạng khác cùng bản chất" — Gemini vẫn tự suy luận theo tinh
thần, KHÔNG so khớp từ khoá cứng (đúng ý Hiếu "nhiều trường hợp khác nữa"). Nhân tiện đọc lại phát hiện
+ fix 1 bug: prompt CŨ dặn trả `is_match=false` cho thread CHƯA hoàn thành → thread biến mất khỏi hàng
chờ duyệt (rơi vào `not_matched`, Hiếu không thấy để tự điền completion tay). Đổi: request thật nhưng
chưa xong → `is_match=true`, `completion_reply_index=null` → HIỆN trong hàng chờ duyệt (FE
`LarkReviewPanel` vốn đã handle `completion_time=null` hiện "chưa xong" từ trước, không cần sửa FE) —
Hiếu dùng nút "Sửa giờ & xác nhận" có sẵn để tự hoàn thành. `DataTable` (component dùng chung toàn
trang) đổi `pageSize` mặc định 50→20, áp cho MỌI bảng trong tab cùng lúc (1 chỗ sửa). tsc PASS. Chưa
verify chất lượng phân loại với dữ liệu Lark thật.
...
- [x] ✅ **v46 `okr_lark_message_log`** (Lark bot real-time capture, s173 — Hiếu đã chạy 2026-08-31)
...
- [x] ✅ **s173 — chạy migration `v46_okr_lark_message_log.sql`** — Hiếu đã chạy 2026-08-31.
- [ ] **s173 — xác nhận đã Kết nối Lark cá nhân** (Creator Settings) + **bot đã add vào group Sales/PIC
  liên quan** — 2 điều kiện còn lại để capture real-time hoạt động (Lark chỉ gửi event cho group bot LÀ
...
- [x] ✅ **s188 Phase 5 — QA UI Quarter Report + Channels trên staging** — đã QA bằng Chrome thật ở s189
  (Creator, data thật): KpiCard/ColInfo tooltip/MomBadge/MonthSubRow toggle/QtSummaryRow+QtTargetRow/
  PivotTable B2C (Quarter Report), B2BCustomerDetail (Channels) đều đúng. PASS.
- [ ] **s189 Phase 5 — QA UI to-gau + My Metrics + B2BTierSection trên staging** (tách cơ học mới, CHƯA
  ...
  (`B2BTierSection` — bảng tier, expand KH theo VN/US, sửa/import/export CH.Cost, target KH per-customer)
  — tsc/vitest/build sạch nhưng chưa xác nhận UI bằng mắt. Đây là phần cuối Phase 5 — sau khi Hiếu QA
  xong cả 4 file, coi Phase 5 hoàn tất hẳn.
