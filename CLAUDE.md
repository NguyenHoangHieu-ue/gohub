...
## Trạng thái hiện tại (2026-09-06, s194+6)
...
| ✅ **s194+6 (2026-09-06) — Tổ Gấu: panel "Câu hỏi CS" + AI search Docs/Notes nhóm + trích nguồn** | Hiếu: CS hay tag người trong ticket/troubleshoot hỏi sản phẩm/policy nhưng câu hỏi "trôi mất", không biết đã update chưa. (1) Bảng mới `chat_questions` (migration `v48_chat_questions.sql` — **Hiếu cần chạy trên Supabase**) — status `chua`/`dang`/`da_xu_ly`, bất kỳ member nào trả lời/đổi trạng thái được (mang tính cộng tác). Panel mới `components/to-gau/questions-panel.tsx`, sub-tab thứ 3 "❓ Câu hỏi" cạnh Docs/Notes trong track "Của nhóm". (2) `searchKB()` (`to-gau/groups/[id]/ai/route.ts`) trước CHỈ tìm `kb_wiki_pages` — nay tìm thêm `chat_docs`+`chat_notes` CỦA CHÍNH GROUP (ILIKE trực tiếp bảng sống, không cần embedding/reindex riêng — lưu Doc/Note mới có hiệu lực ngay lập tức, đúng yêu cầu "bot cập nhật ngay"). (3) System prompt Gấu Tổ thêm rule bắt buộc trích nguồn `(Nguồn: [Wiki]/[Tài liệu nhóm]/[Ghi chú nhóm] ...)` cuối câu trả lời để người hỏi bấm sang tab tương ứng kiểm chứng lại — đáp ứng yêu cầu "kiểm chứng ngay câu trả lời". tsc + lint (0 lỗi mới) + vitest (185/185) PASS. **Hiếu cần: chạy migration v48, rồi QA panel Câu hỏi + hỏi AI 1 câu có đáp án trong Docs/Notes group để xác nhận trích nguồn đúng.** |
...
- [ ] **s194+6 — chạy migration `v48_chat_questions.sql`** trên Supabase + QA panel "❓ Câu hỏi" (Tổ Gấu →
  Tài liệu → Của nhóm) + thử hỏi AI Gấu Tổ 1 câu có đáp án nằm trong Docs/Notes của group để xác nhận AI
  trích đúng nguồn `(Nguồn: ...)`.
...
**v46** `okr_lark_message_log` — tất cả v44-v46 Hiếu đã xác nhận chạy. · **v47** `analytics_query_cache.deps` (Hiếu đã xác nhận chạy 2026-09-05) · **v48** `chat_questions` (⚠️ Hiếu CHƯA xác nhận chạy — xem checklist trên).
