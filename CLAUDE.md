...
## Trạng thái hiện tại (2026-09-04, s188)

| | |
|---|---|
| Branch làm việc | `staging` (làm việc ở đây, merge main **CHỈ khi Hiếu yêu cầu RÕ RÀNG** trong chính tin nhắn đó) |
| tsc + `next build` | PASS |
| ⏳ **s188 (2026-09-04) — Phase 5 plan rebuild: tách cơ học component FE (chưa xong hết 4 file)** | Hiếu chốt: CHỈ tách cơ học (move nguyên khung, KHÔNG gộp khối JSX khác nhau) vì máy dev không có `.env.local` → không chạy được dev server để verify UI bằng mắt, tsc là lưới an toàn duy nhất. Đã xong: `quarterly/page.tsx` 3077→2688 dòng (tách KpiCard/TableHead/ColInfo/MomBadge/MonthSubRow/QtSummaryRow/QtTargetRow/PivotTable sang `components/quarterly/*` + types/format helpers sang `lib/quarterly-types.ts`/`lib/quarterly-format.ts`); `channels/page.tsx` 1944→1843 dòng (tách B2BCustomerDetail sang `components/channels/`). **CHƯA làm**: `B2BTierSection` (~1080 dòng, còn lại trong quarterly/page.tsx, quá lớn cho 1 pass) — `to-gau/[id]/page.tsx` (2843 dòng, đã có sẵn SettingsModal/DocsPanel/NotesPanel/WikiPanel định nghĩa riêng trong file, cùng pattern tách được) — `my-metrics/page.tsx` (1809 dòng, đã có sẵn LarkReviewPanel/EvidenceCard/SkuScanSection/BegauInsightsSection/LarkConfigModal). tsc + vitest 177/177 PASS sau mỗi bước. Plan: `C:\Users\nhhie\.claude\plans\effervescent-zooming-wilkinson.md`. **Cần Hiếu**: QA kỹ 2 tab Quarter Report + Channels trên staging (đây là refactor KHÔNG verify được bằng mắt ở máy dev — dù tsc sạch, vẫn nên xác nhận UI y hệt trước khi yên tâm) trước khi làm tiếp 2 file còn lại. |
