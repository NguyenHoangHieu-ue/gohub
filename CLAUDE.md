...
- [ ] **s191 — chạy migration `v47_cache_deps.sql`** trên Supabase (thêm cột `deps` vào
  `analytics_query_cache`) — cache B2B cost vẫn chạy đúng nếu chưa chạy (cột thiếu → ghi/đọc `deps` lỗi
  âm thầm, cache rơi về "luôn miss", chỉ chậm hơn chứ không sai số) nhưng nên chạy sớm cho đúng thiết kế.
...
**v46** `okr_lark_message_log` — tất cả v44-v46 Hiếu đã xác nhận chạy. · **v47** `analytics_query_cache.deps`
(⚠️ CHƯA xác nhận Hiếu đã chạy — xem checklist trên).
