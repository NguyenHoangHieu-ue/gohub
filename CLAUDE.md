...
| | |
|---|---|
| ⏳ **s203+1 (2026-09-21) — 3HK Data Usage: thêm "Export đầy đủ (theo tháng)" nhiều sheet + cột kỳ/tháng (đã push staging `94d78d1d`+sửa thiếu cột, CHƯA merge main, chờ Hiếu thử file)** | Hiếu: export nhiều tháng cần ngày/tháng để phân biệt + thống kê. Export Records cũ gom 1 dòng/bundle cho CẢ kỳ nên không biết tháng nào. Thêm nút **"Export đầy đủ (theo tháng)"** (header bảng "Average Usage by SKU", `exportMonthly` trong `3hk-usage/page.tsx`): .xlsx nhiều sheet (Tổng quan · Theo SKU Type · Theo SKU · Unlimited theo mã · Zone x Tháng), mỗi sheet có `Cả kỳ` + từng tháng, đủ cột như UI (mã loại gói, kế hoạch/thực tế GB/ngày/SIM, Efficiency...), theo tab + Search. Bản đầu chỉ có Tháng×SKU nên thiếu cột → Hiếu báo, đã sửa. Export Records thêm cột `Tháng`/`Kỳ từ`/`Kỳ đến`. ⚠️ (iccid, order_code) tính riêng TỪNG tháng có usage → tổng Active SIMs các tháng của 1 SKU > số bảng SKU (1 SIM = 1 lần cả kỳ) — đúng thiết kế, không phải bug. Đã verify sống: SQL chạy trên staging (kỳ T7-T8/2026 → 2.809 dòng, đủ 2 tháng), nút hiện sau deploy. Chưa bấm tải file thật (cần Hiếu mở xem). tsc PASS. Wiki `analytics-3hk-usage.md` §9. |
