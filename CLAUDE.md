...
## Trạng thái hiện tại (2026-09-20, s202)

| | |
|---|---|
| ⏳ **s202 (2026-09-20) — Bridge ghi nhận thiết bị + gộp Giám sát Dữ liệu vào "Dữ liệu & API" + SQL Query kiểu Power BI (Query Studio) — đã push staging, chưa merge main** | **Bridge**: extension 1.1.0 sinh `deviceId`, gửi OS/arch/Chrome/ext version/múi giờ/email tài khoản Chrome (`identity.email`); server (`lib/bridge-device.ts` `authBridge`) bắt buộc `X-Device-Id`, ghi IP, gắn device+IP vào từng lệnh claim; bản extension cũ bị 400 tới khi reload. Migration `v61_browser_bridge_devices.sql` (**Hiếu phải chạy + Reload schema**). Trang Bridge: user thấy máy của mình + Thu hồi; creator xem toàn bộ user + nhật ký lệnh (`bridge-devices.tsx`, API `bridge/devices`). Chrome KHÔNG cho extension đọc tên máy. **Giám sát Dữ liệu**: 3 mục thành 1 trang liền (`data-health-view.tsx`), gộp vào `/analytics/creator/devtools` tab con "Giám sát" (chỉ creator thấy); `/creator/data-health` chỉ còn redirect; nav đổi tên "Dữ liệu & API". **Query Studio** (`devtools/query-studio*.tsx`, `lib/query-studio.ts`): Data pane + editor + Visualizations (Table/Column/Bar/Line/Area/Donut/Card, Axis/Legend/Values + Sum/Avg/Count..., Top N, Stacked, kéo-thả), bảng sort/lọc/tổng cột, lịch sử, toàn màn hình; `/api/admin/sql-query` cắt 10.000 dòng. tsc + vitest 321 PASS; đã QA Chrome staging (redirect, 3 khối, chạy SQL → Line/Column+Legend/Table). Chi tiết: wiki `analytics-devtools.md`, `analytics-data-health.md`, `analytics-creator-ai.md` §Bridge s202. |
...
- [ ] **s201 — Theo dõi sau merge main (2026-09-19)**: (a) cron sync 01:00 UTC 20/09 là lần đầu chạy `core` bằng code mới (có tự thêm vendor + xoá cache
  ...
  có dòng `📍 Tab: …` đúng không (bước `jq` trong `notify-release.yml` chưa thử được ở máy dev); (d) nếu Catalogue chưa thấy thay đổi → bấm "Tải lại" (cache 30').
- [ ] **s201 — Bảo mật**: `tmp.txt` (untracked, gốc repo) chứa secret thật — **Vercel token đã lộ vào hội thoại Claude** → thu hồi (rotate) token đó + xoá/gitignore `tmp.txt`.
- [ ] **s201 — Xác nhận dữ liệu vendor**: `ref_vendors` ghi `GB` = **Gighub**, wiki business `ma-sku.md` ghi GB = **WorldMove** — đối chiếu cái nào đúng rồi sửa bên sai.
  Tên tạm do sync tự thêm (banner xanh ở Catalogue, chỉ admin/creator thấy) → sửa ở Admin → Import ref data (bảng `ref_vendors`).
...
- [x] **s201 — `VACUUM (ANALYZE) items;`** Hiếu đã chạy (count exact 8,9s → 1,5s).
