...
bấm "KH đang mua", 1 trong 4 tier, hoặc 1 trong 4 ô vòng đời (mới / cũ tiếp tục / cũ quay lại sau gián đoạn / cũ chưa quay lại) → bảng khách hàng ngay bên dưới (bấm lại / X để đóng, sắp theo doanh thu, tối đa 300 dòng).
Route `squad-progress` thêm mỗi khách `lifecycle_group` (`new|continuing|returning`) + `prev_revenue`; danh sách "chưa quay lại" nay ĐỦ (≤300/squad, trước 30) kèm `sales_pic` + `first_order_date` — tên KH tra bằng `fetchCustomerNames` cho cả danh sách (1 query IN, không N+1). Khối `<details>` cũ đã bỏ.

**2. Tab mới "Performance"** (cạnh Tổng quan / Squad Progress; `components/quarterly/company-performance-view.tsx`, logic thuần + test ở `lib/quarterly-company-view.ts` / `__tests__/quarterly-company-view.test.ts`):
bảng cho **ALL / B2B / B2C** theo mẫu Excel của Hiếu — Q trước trong năm (+%QoQ) | tháng đang xem (tháng chạy = Pro-rata) + quý + %QoQ | target từng tháng quý sau + tổng + "Target +%QoQ" | cột cả năm. Dòng Revenue / GP / GP% / CM1 / CM1% / 3HK/Datapool / 3HK%.
- **Nguồn số thực tế = route `quarterly-report`** (đúng số bảng "Tổng hợp theo Tháng" của Tổng quan, tháng chạy dùng factor GATE 7 ngày — khác Squad Progress dùng factor không gate). Tab tự gọi lại route đó cho từng quý trước (Q1…Q(n-1), mỗi quý 1 request, route đã cache) với cùng filter VN/US/Ship/Nội bộ; quý đang xem dùng `report` của page. `row()` trong route thêm `hk3Rev` cho total/B2B/B2C (FE có fallback hk3Pct×revenue nếu response cũ còn trong CDN).
- **%QoQ = thay đổi TƯƠNG ĐỐI (mới−cũ)/|cũ|, kể cả dòng %** (30%→23% = −23%) đúng file Excel; bảng theo tháng của Squad đã đổi từ "pp" sang cách này cho thống nhất.
- **Cột cả năm** = các quý trước + quý đang xem + target quý sau (chỉ hiện khi xem Q3/Q4; Q3 cần nhập đủ target Q4 mới hiện số, chưa đủ hiện "—").
- **Target tháng cấp công ty** (nhập ở nút "Target Qx-yyyy", admin/creator): `app_settings.company_monthly_targets`, khoá `Q4_2026` → `{ALL|B2B|B2C: {rev,gp,cm1,hk3rev: number[3]}}`; API `GET/POST /api/analytics/company-monthly-targets` (POST ghi đè từng segment gửi lên). ALL bỏ trống = tự cộng B2B+B2C từng tháng/metric; ALL nhập riêng thì GIỮ NGUYÊN (file Excel của Hiếu: ALL Q4 ≠ B2B+B2C). ⚠️ Target tháng này ĐỘC LẬP với target quý B2B/B2C (Turso `target_planning_quarter`) và target squad — nhập 2 nơi không tự đồng bộ (chưa có fallback Σ3 tháng như squad).

**s209 QA (2026-09-25) — đối chiếu độc lập + sửa lỗi:**
- **Số liệu đã đối chiếu SQL thô `gohub_dw`** (đúng bộ lọc báo cáo: loại ship, INTERNAL-TRANSACTION, KH INACTIVE, danh sách loại trừ): Revenue/GP/3HK từng tháng × B2B/B2C Q1-Q3 khớp tuyệt đối (lệch ≤1đ do làm tròn; T9 khớp khi SQL cắt đúng mốc dữ liệu 24/09 và Pro-rata ×30/24 = ×1,25). Squad 1/2: revenue/GP/3HK từng tháng khớp SQL theo PIC; phân loại KH (mới 56/2, tiếp tục 128/14, quay lại 4/0, chưa quay lại 59/0) khớp SQL từng khách, `prev_revenue` khớp doanh thu Q2. Tab Performance: 168 ô (giá trị, %, %QoQ) khớp tính độc lập từ báo cáo.
