...
| **Sync GoHub API → Supabase** (`backend/data_sync/sync.py`, GitHub Actions `sync.yml`, 01:00 UTC) | ⚠️ Fix s198+11 (2026-09-15): verify qua GitHub Actions run history — cron thành công lần cuối **2026-07-20**, sau đó 100% fail (429 rate-limit GoHub API, 4 resource products/skus/listings/items fetch song song `ThreadPoolExecutor(max_workers=4)` cộng dồn request rate) suốt 57 ngày → products/skus/listings/items Supabase đứng yên từ giữa tháng 7. Fix: mount `urllib3.Retry` (respect `Retry-After`, backoff luỹ thừa, retry 429/5xx) vào `GohubClient.session`; nâng `timeout-minutes` 20→90 (20' thêm 2026-08-09 defensive chung, quá ngắn — lịch sử lúc thành công từng mất tới 59'). **Cron LUÔN chạy theo branch `main`** (khác Vercel — không theo staging), fix chưa có hiệu lực tới khi merge main.  **s201 (2026-09-19) — tìm ra nguyên nhân gốc THẬT sau khi 20 run vẫn 0 thành công**: (1) `sync.py` tải CẢ 4 bảng rồi mới ghi, trong khi **items (233k dòng) mất ~82 phút** vì server GoHub API cắt cứng **200 dòng/trang** dù client xin `limit=1000` (log `+200`) và client tải TUẦN TỰ 1.167 trang × ~4,7s → run 09-18 (timeout 90') dừng ở 208k/233k, không ghi gì ⇒ products/skus/listings (chỉ vài phút) cũng đứng yên từ 2026-07-20 (sync_log: products 973 vs API 1.084, skus 12.892 vs 15.936, items 203.726 vs 233.324). (2) 502 kéo dài (09-17) vượt hết retry ⇒ chết cả run. (3) `print` không flush ⇒ log ra từng cục 13'. **Fix**: `sync.py core|items|all` — CORE (products→skus→listings) upsert NGAY khi tải xong từng bảng; ITEMS job riêng (`sync.yml` 2 job: `core` timeout 30', `items` timeout 150', `needs: core`, `if: always()`); `_fetch_all` lấy page size thực từ trang 1 rồi tải trang còn lại song song có giới hạn (`GOHUB_CORE_WORKERS`=3, `GOHUB_ITEMS_WORKERS`=4), ghép đúng thứ tự trang, kiểm `len==total`; vòng thử lại cấp trang + cooldown dùng chung giữa các luồng; `_TimeoutAdapter` (10s connect/60s read — trước không có timeout); log flush. Test: `backend/data_sync/test_pagination.py` (pytest, 8 ca, session giả). ⚠️ `.gitignore` có `backend/` nên file test MỚI cần `git add -f`. Backfill: `gh workflow run sync.yml --ref staging` sau khi push (workflow đã có trên main nên dispatch được). |
| **Thông báo Lark khi có tính năng mới lên production** (s200+8) | Hiếu yêu cầu: mỗi khi merge tính năng
  mới lên `main`, tự động báo 1 group Lark riêng do Hiếu setup. Đã hỏi 3 quyết định qua AskUserQuestion:
  (1) chỉ báo khi merge `main` (KHÔNG báo mỗi lần push staging, tránh spam lúc code còn dang dở); (2) nội
  dung do Gemini tóm tắt lại commit message (vốn viết chi tiết kỹ thuật theo rule CLAUDE.md) thành thông
  báo ngắn cho người không rành code (bỏ mã session/tên file/thuật ngữ, gộp commit cùng chủ đề, tối đa 6
  dòng — commit thuần dọn dẹp/docs không có gì đáng báo thì trả rỗng, không gửi); (3) group MỚI riêng,
  tách khỏi group đang nhận thông báo sync/SKU đổi giá (`lark_notify_chat_id`).
  **Luồng**: `.github/workflows/notify-release.yml` (trigger `on: push branches:[main]`) gom
  `github.event.commits` → `POST /api/notify/release` (auth `Bearer $MCP_SECRET`, cùng secret với
  `/api/notify/lark`) → `summarizeReleaseCommits()` (`lib/release-notify.ts`, Gemini `gemini-3.8-flash`
  `thinkingLevel:"low"`, cùng pattern `weekly-report/narrative.ts`) → gửi group Lark đọc từ
  `app_settings.lark_release_chat_id` (key MỚI, tách hẳn `lark_notify_chat_id`).
  **Đặt group đích**: lệnh Lark mới `/set-release-channel` (`api/lark/events/route.ts`, chỉ admin/creator,
  @mention bot kèm lệnh trong group muốn nhận) — upsert `lark_release_chat_id` = chat_id group đó (ghi đè
  được nhiều lần, KHÔNG dùng `ignoreDuplicates` như `lark_notify_chat_id` — đổi group đích bất cứ lúc nào
