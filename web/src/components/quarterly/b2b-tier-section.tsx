...
export function B2BTierSection({ b2bTiers, loading, months, allMonths, region, onRegionChange, expanded, onToggle, canEditCost, isCreator, onSaved, notify, quarterLabel, qFactor = 1, summary = [], futureScale = 1 }:
  ...
  const exportTierTable = async () => {
    ...
    const headers: string[] = ["Nhóm", "Số KH"]
    ...
    const rows: (string | number)[][] = tiers.map((tierRaw: any) => {
      const tier = pickView(tierRaw)
      const row: (string | number)[] = [tierRaw.tier, tier.customerCount]
      visibleMonths.forEach(m => {
        const d = tier.months.find((x: any) => x.month === m)
        if (!d?.hasData) { EXPORT_SUB_LABELS.forEach(() => row.push("")); return }
        row.push(
          Math.round(d.revenue), Math.round(d.gm), d.cc > 0 ? Math.round(d.cc) : "",
          Math.round(d.cm1), Number((d.cm1Pct ?? 0).toFixed(1)), "",
      ...
      row.push(qPrRev, qPrGm, qPrCc > 0 ? qPrCc : "", qPrCm1, qPrCm1Pct, tierQoQ, Math.round(tier.totalHk3Rev ?? 0), Number((tier.totalHk3Pct ?? 0).toFixed(1)))
      return row
    })
