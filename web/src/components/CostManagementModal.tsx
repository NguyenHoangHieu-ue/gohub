...
export const CostManagementModal: React.FC<CostManagementModalProps> = ({ isOpen, onClose, onSave, initialMonth }) => {
  ...
  const fetchData = async (month: string) => {
    try {
      ...
      if (subRes.ok) setVnEcomSubChannels(await subRes.json())
    } catch (err) {
  ...
  const handleSaveGroupCost = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewGroupCost({ group_name: "B2C", month: costMonth, item_name: "", amount: 0, note: "" })
        fetchData(costMonth)
  ...
  const toggleVNEcomMode = async (mode: "total" | "subchannels") => {
    try {
      const res = await fetch("/api/channel-cost-settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ channel: "VN-Ecom", month: costMonth, mode, updatedBy: "system" }),
      })
      if (res.ok) setCostSettings(prev => ({ ...prev, "VN-Ecom": mode }))
    } catch (err) {
  ...
  const handleSaveCost = async (channel: string) => {
    ...
    setSaveSuccess(null)
    try {
      if (channel === "VN-Ecom" && costSettings["VN-Ecom"] === "subchannels") {
        const promises = vnEcomSubChannels.map(sc => {
          const subChannelName = `VN-Ecom - ${sc}`
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      baseChannels.forEach(channel => {
        if (channel === "VN-Ecom" && costSettings["VN-Ecom"] === "subchannels") {
          vnEcomSubChannels.forEach(sc => channelsToSave.push(`VN-Ecom - ${sc}`))
        } else {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="p-6 border-b border-slate-100 flex flex-col gap-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl w-fit">
            {(["b2b", "b2c", "group"] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveCostTab(tab)}
                className={cn(
                  "px-6 py-2 text-sm font-bold rounded-lg transition-all",
                  activeCostTab === tab ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"
                )}
              >
                {tab === "b2b" ? "B2B Channels" : tab === "b2c" ? "B2C Channels" : "Group Costs"}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-6">
            {activeCostTab === "group" ? (
              <div className="space-y-6">
                <div className="bg-blue-50 rounded-2xl p-6 border border-blue-100">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {newGroupCost.id && (
                      <button
                        onClick={() => setNewGroupCost({ group_name: "B2C", month: costMonth, item_name: "", amount: 0, note: "" })}
                        ...
                        <div key={cost.id} className="bg-white border border-slate-100 rounded-2xl p-4 flex items-center justify-between hover:shadow-md transition-shadow">
                          ...
                          <div className="flex items-center gap-6">
                            ...
                            <div className="flex items-center gap-1">
                              <button onClick={() => handleEditGroupCost(cost)} className="p-2 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-xl transition-all" title="Edit">
                                <Edit2 className="w-5 h-5" />
                              </button>
                              <button onClick={() => cost.id && handleDeleteGroupCost(cost.id)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="Delete">
                                <X className="w-5 h-5" />
                              </button>
                            </div>
                          </div>
              ...
              (activeCostTab === "b2c" ? B2C_CHANNELS_LIST : B2B_CHANNELS_LIST).map((channel) => {
                const isVNEcom = channel === "VN-Ecom"
                const isSubchannelMode = isVNEcom && costSettings["VN-Ecom"] === "subchannels"
                    ...
                    <div key={chanName} className={cn("bg-slate-50 rounded-2xl p-4 border border-slate-100", displayName && "ml-8 border-l-4 border-l-blue-200")}>
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex flex-col">
                          ...
                          {isVNEcom && !displayName && (
                            <div className="flex items-center gap-2 mt-1">
                              <button onClick={() => toggleVNEcomMode("total")} className={cn("text-[10px] px-2 py-0.5 rounded-full font-bold transition-all", !isSubchannelMode ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-500")}>
                                Total Mode
                              </button>
                              <button onClick={() => toggleVNEcomMode("subchannels")} className={cn("text-[10px] px-2 py-0.5 rounded-full font-bold transition-all", isSubchannelMode ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-500")}>
                                Sub-channel Mode
                              </button>
                            </div>
                          )}
                        </div>
                        <button onClick={() => handleSaveCost(chanName)} disabled={savingCosts} className="flex items-center gap-2 px-4 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
                          <Save className="w-3 h-3" />
                          Save
                        </button>
                      </div>
                  ...
                  return (
                    <div key={channel} className="space-y-4">
                      <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100 flex items-center justify-between">
                        ...
                        <button onClick={() => toggleVNEcomMode("total")} className="text-xs bg-white text-blue-600 px-3 py-1 rounded-lg border border-blue-200 font-bold hover:bg-blue-50 transition-all">
                          Switch to Total Mode
                        </button>
                      </div>
                      {vnEcomSubChannels.map(sc => renderChannelRow(`VN-Ecom - ${sc}`, sc))}
                    </div>
                  )
                }
