...
OPERATORS (PM API: /api-pull/gohub-cloud/operators → Supabase: pm_operators):
- code: mã operator (MOBIFONE, SKYFI, TRUEMOVE, ...)
- name: tên đầy đủ | description: mô tả
- image_url: logo/hình nhà mạng
- country: mã ISO nước (VN, TH, KR, ...)
- category_codes: danh sách mã category liên kết (array)

PRICE_LISTS (PM API: /api-pull/gohub-cloud/price-lists → Supabase: pm_price_lists):
- type: ext (external/partner) | itn (internal)
- tenant: VN / US (có thể null)
- code: mã 3 ký tự (LV2, TV3, BSP, GHI, OPS, ...)
- channel / channel_code: kênh bán
- label: nhãn hiển thị | description: mô tả
- listing_type: mã loại listing
- sort_order: thứ tự sắp xếp
- is_active: true/false

SKU_VAT (PM API: /api-pull/gohub-cloud/sku-vat → Supabase: sku_vat):
- Chỉ tenant VN. Product Type D/E (SIM Frame/SIM Full) luôn vat_status=No.
- sku_code: 13 ký tự | vendor_code | product_code | product_type (label)
- vat_status: Yes / No (effective — giống màn SKU VAT Admin)
- name_vn: tên sản phẩm tiếng Việt
- vat_unit: VND / USD | vat_price: giá VAT | vat_tax_rate: "8%" / "10%"
- Endpoint /sku-vat/sku-codes: trả danh sách sku_code có vat_status=Yes (phân trang)
