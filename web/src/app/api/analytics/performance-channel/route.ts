...
import { getAnalyticsSource, getDateFilter, getPrevDateFilter, getStrategicPartnersList, getGroupCaseSQL, cachedQuery, CACHE_HEADERS, analyticsGuard } from "@/lib/analytics-helpers"
...
export async function GET(req: NextRequest) {
  ...
  const cacheKey    = `perf-channel:${startDate}:${endDate}:${dateColumn}:${companyCode}`
    ...
    const data = await cachedQuery(cacheKey, async () => {
      ...
      const rows = await queryAnalytics<Record<string, string>>(
        `WITH current_period AS (
           ...
           WHERE ${filter} GROUP BY "channel_name", business_group
         ),
         previous_period AS (
           SELECT COALESCE(s.channel_name,'Unknown') as "channel_name",
                  SUM(f.${source.revenueCol}) as grossRevenue
           FROM ${source.mainTable} f
           LEFT JOIN dim_order_source s ON f.order_source_code = s.code
           WHERE ${prevFilter} GROUP BY "channel_name"
         )
         SELECT c."channel_name" as "group", c.business_group, c.totalOrder, c.unitSold, c.grossRevenue,
                CASE WHEN COALESCE(p.grossRevenue,0)=0 THEN 0
                     ELSE ((c.grossRevenue-p.grossRevenue)/p.grossRevenue)*100 END as mom
         FROM current_period c LEFT JOIN previous_period p ON c."channel_name"=p."channel_name"
         ORDER BY CASE WHEN c.business_group LIKE 'B2B%' THEN 1 WHEN c.business_group='B2C' THEN 2 ELSE 3 END, c.grossRevenue DESC`
      )
