...
// Target CM1 và 3HK% per-KH per-quý — lưu Supabase b2b_customer_targets.
...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabaseAdmin
    ...
    .select("customer_code, target_cm1, target_3hk_pct")
    .eq("quarter", quarter)
    .eq("year", year)

  const targets: Record<string, { cm1: number; thk: number }> = {}
  ;(data ?? []).forEach(r => {
    targets[r.customer_code] = { cm1: Number(r.target_cm1) || 0, thk: Number(r.target_3hk_pct) || 0 }
  })
...
export async function POST(req: NextRequest) {
  ...
  const { quarter, year, customer_code, target_cm1, target_3hk_pct } = body
