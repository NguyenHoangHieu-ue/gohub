...
export default function B2BPerformancePage() {
  ...
  const [wholesaleSort, setWholesaleSort] = useState<{ key: keyof PerformanceData; direction: "asc" | "desc" }>({
    key: "revenue",
    direction: "desc",
  })
  ...
  const getProjectionFactor = () => {
    ...
    let targetDays
    if (isCurrentMonth) {
      targetDays = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate()
    } else {
      targetDays = daysElapsed
    }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      qp.append("dateColumn", dateColumn)
      qp.append("comparisonType", "previous_period")

      // Parallel fetch (speed), single loading state (y đúc render)
      const [b2bKpis, b2bPerfCustomer, strategicPerf, trend, tiersData] = await Promise.all([
        fetch(`/api/analytics/b2b/kpis?${qp.toString()}`).then(r => r.ok ? r.json() : []),
        fetch(`/api/analytics/b2b/performance?${qp.toString()}&groupBy=customer`).then(r => r.ok ? r.json() : []),
        fetch(`/api/analytics/b2b/strategic-performance?${qp.toString()}`).then(r => r.ok ? r.json() : []),
        fetch(`/api/analytics/b2b/trend?startDate=${startDate}&endDate=${endDate}&dateColumn=${dateColumn}&granularity=${granularity}`).then(r => r.ok ? r.json() : []),
      ...
      setPartnerTiers(tiersData || { Strategic: [] })
  ...
  const sortData = (data: PerformanceData[], config: any) =>
    [...data].sort((a: any, b: any) => config.direction === "asc" ? (a[config.key] > b[config.key] ? 1 : -1) : (a[config.key] < b[config.key] ? 1 : -1))
  ...
  const getFilteredOtherTiers = () => {
    ...
    nonStrategicRaw.forEach(p => {
      ...
      if (existing) {
        ...
        existing.margin += (p.margin || 0)
        existing.units += (p.units || 0)
        existing.gpm2 += (p.gpm2 || 0)
        if (p.sub_channels && p.sub_channels.length > 0) {
          ...
          p.sub_channels.forEach(sub => {
            ...
            if (existingSub) {
              existingSub.revenue += (sub.revenue || 0)
              existingSub.margin += (sub.margin || 0)
              existingSub.units += (sub.units || 0)
              existingSub.gpm2 += (sub.gpm2 || 0)
            } else {
                ...
                {combinedKpis.map((kpi, idx) => (
                  <div key={`actual-${idx}`} className="bg-white p-4 lg:p-5 rounded-2xl shadow-sm border border-slate-200">
                    <div className="flex items-center justify-between mb-3">
                      <div className={cn("p-2 rounded-xl", idx === 0 ? "bg-blue-50 text-blue-600" : idx === 1 ? "bg-emerald-50 text-emerald-600" : idx === 2 ? "bg-purple-50 text-purple-600" : idx === 3 ? "bg-orange-50 text-orange-600" : "bg-indigo-50 text-indigo-600")}>
                        {kpi.icon}
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">Revenue (Bar)</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-0.5 bg-indigo-600 relative after:content-[''] after:absolute after:top-1/2 after:left-1/2 after:-translate-x-1/2 after:-translate-y-1/2 after:w-1.5 after:h-1.5 after:bg-indigo-600 after:rounded-full"></div>
                      <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">GPM2 (Line)</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => {
                        ...
                        if (isProjectable) {
                          columns.push({ label: "Projected Revenue", key: "projected_revenue" })
                          columns.push({ label: "Projected GP", key: "projected_margin" })
                          columns.push({ label: "Projected GPM2", key: "projected_gpm2" })
                        }
                        columns.push(
                          ...
                          { label: "GPM2", key: "gpm2" },
                          { label: "GPM2 %", key: "gpm2_percent" },
                        )
                        const exportData = strategicPerformance.map(d => ({
                          ...d,
                    ...
                    <div className="flex items-center gap-2 px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-[10px] font-bold">
                        ...
                        return (
                          <>
                                                    ...
                                                    return (
                                                      <div key={category} className="bg-white/60 p-2.5 rounded-xl border border-indigo-100 shadow-sm backdrop-blur-sm">
                                                        ...
                                                        <div className="flex items-baseline justify-between">
                                                          <p className="text-xs font-bold text-slate-700">{formatNumber(Math.round(amount))}</p>
                                                        </div>
                                                      </div>
                                                    )
                                                  })}
                            ...
                            {strategicPerformance.length > 0 && (() => {
                              const totals = strategicPerformance.reduce((acc, curr) => ({
                                revenue: acc.revenue + curr.revenue,
                                units: acc.units + curr.units,
                                margin: acc.margin + curr.margin,
                                gpm2: acc.gpm2 + curr.gpm2,
                              }), { revenue: 0, units: 0, margin: 0, gpm2: 0 })
                            ...
                            {strategicPerformance.length === 0 && (
                              <tr>
                                <td colSpan={7} className="px-8 py-12 text-center text-slate-400 italic">No strategic partners tracking data available for the selected period.</td>
                              </tr>
                            )}
                          </>
                        )
                ...
                <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const exportData = nonStrategic.map(d => ({
                        ...d,
                        projected_revenue: Math.round(d.revenue * projectionFactor),
                        projected_margin: Math.round(d.margin * projectionFactor),
                        projected_gpm2: Math.round(d.gpm2 * projectionFactor),
                    ...
                    <thead>
                      <tr className="bg-slate-50/50">
                        {([
                          ...
                          { key: "margin", label: "Gross Profit", align: "right" },
                          { key: "margin_percent", label: "Margin %", align: "right" },
                          { key: "gpm2", label: "GPM2", align: "right" },
                          { key: "gpm2_percent", label: "GPM2 %", align: "right" },
                        ] as const).map(col => (
                          <th
                            key={col.key}
                            className={cn("px-8 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest cursor-pointer hover:bg-slate-100 transition-colors", col.align === "right" && "text-right")}
                            onClick={() => setWholesaleSort({ key: col.key as keyof PerformanceData, direction: wholesaleSort.key === col.key && wholesaleSort.direction === "desc" ? "asc" : "desc" })}
                          >
                            <div className={cn("flex items-center", col.align === "right" && "justify-end")}>{col.label} <SortIcon sort={wholesaleSort} column={col.key} /></div>
                          </th>
                        ))}
                      </tr>
                    </thead>
                                    ...
                                    <tr className="bg-slate-50/80">
                                      <td colSpan={7} className="px-8 py-3">
                                        <div className="space-y-4">
                                          ...
                                          <div className="grid grid-cols-4 gap-4">
                                            {(["ads", "platformFee", "sponsorProducts", "media"] as const).map((category) => {
                                              const amount = row.cost_breakdown?.[category] || monthlyCosts[row.name]?.[category] || 0
                                              return (
                                                <div key={category} className="bg-white p-2.5 rounded-xl border border-slate-200 shadow-sm">
                                                  ...
                                                  <div className="flex items-baseline justify-between">
                                                    <p className="text-xs font-bold text-slate-700">{formatNumber(Math.round(typeof amount === "number" ? amount : 0))}</p>
                                                  </div>
                                                </div>
                                              )
                                            })}
