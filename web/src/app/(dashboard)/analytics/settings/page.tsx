      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {Object.keys(tiers).map(tier => {
            ...
            return (
              <div key={tier} className="flex items-start gap-4 px-6 py-4">
                <div className="w-28 shrink-0 pt-0.5">
                  <span className="inline-block px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded text-xs font-bold">{tier}</span>
                  <p className="text-[10px] text-slate-400 mt-1">{list.length} đối tác</p>
                </div>
                <div className="flex-1 flex flex-wrap gap-1.5 items-center min-h-[28px]">
                  {list.map(name => (
                    <span key={name} className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 text-slate-700 rounded text-xs font-medium">
                      {name}<button onClick={() => removePartner(tier, name)} className="text-slate-400 hover:text-rose-500 ml-0.5"><X className="w-3 h-3" /></button>
                    </span>
                  ))}
                  {list.length === 0 && <span className="text-xs text-slate-300 italic">Chưa có</span>}
                  {addingTier === tier ? (
                    <select autoFocus value="" onChange={e => { if (e.target.value) { addPartner(tier, e.target.value); setAddingTier(null) } }} onBlur={() => setAddingTier(null)}
                      className="bg-white border border-blue-300 rounded px-2 py-0.5 text-xs focus:ring-1 focus:ring-blue-500 outline-none cursor-pointer">
                      <option value="">Chọn đối tác…</option>
                      {availablePartners.filter(p => !list.includes(p)).map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                  ) : (
                    <button onClick={() => setAddingTier(tier)} className="inline-flex items-center gap-0.5 px-2 py-0.5 text-xs text-blue-600 hover:bg-blue-50 rounded border border-dashed border-blue-300">
        ...
        <p className="px-6 py-3 text-[11px] text-slate-400 border-t border-slate-100">Quyết định phân loại B2B-Strategic vs Non-Strategic (BOD/B2B/Dashboard/All-Time).</p>
      </div>
