...
import {
  ...
  Bookmark, BookmarkCheck, X, BarChart3, Gauge,
} from "lucide-react"
import { cn } from "@/lib/utils"
import { DataLineageMap } from "@/components/data-lineage-map"

const panelLoading = () => <div className="h-96 animate-pulse bg-slate-100 rounded-2xl" />
const QueryStudio = dynamic(() => import("./query-studio").then(m => m.QueryStudio), { ssr: false, loading: panelLoading })
const DataHealthView = dynamic(() => import("./data-health-view").then(m => m.DataHealthView), { ssr: false, loading: panelLoading })
...
export default function CreatorDevToolsPage() {
  ...
  return <React.Suspense fallback={null}><DevTools isCreator={freshRole === "creator"} /></React.Suspense>
}

type DevTab = "monitor" | "api" | "db" | "sql" | "lineage"
const DEV_TABS: DevTab[] = ["monitor", "api", "db", "sql", "lineage"]

function DevTools({ isCreator }: { isCreator: boolean }) {
  // s202: gộp tab "Giám sát Dữ liệu" (creator-only) vào đây; SQL Query làm lại kiểu Power BI (Query Studio).
  // Admin (creator bật Tab Visibility) vẫn dùng API/DB/SQL/Data Map; riêng "Giám sát" chỉ creator thấy.
  const params = useSearchParams()
  const wanted = params.get("tab") as DevTab | null
  const [tab, setTab] = useState<DevTab>(() =>
    wanted && DEV_TABS.includes(wanted) && (wanted !== "monitor" || isCreator) ? wanted : isCreator ? "monitor" : "api")

  const tabs: { key: DevTab; label: string; Icon: React.ComponentType<{ className?: string }>; creatorOnly?: boolean }[] = [
    { key: "monitor", label: "Giám sát", Icon: Gauge, creatorOnly: true },
    { key: "sql", label: "SQL Query", Icon: BarChart3 },
    { key: "api", label: "API Tester", Icon: Terminal },
    { key: "db", label: "Database", Icon: Database },
    { key: "lineage", label: "Data Map", Icon: GitBranch },
  ]

  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen max-w-[1600px] mx-auto">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center shadow-sm">
          <Crown className="w-5 h-5 text-white" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Dữ liệu &amp; API</h1>
          <p className="text-slate-500 text-sm">Giám sát dữ liệu, chạy SQL dựng biểu đồ, kiểm thử API và duyệt database Supabase/Turso. Chỉ Creator/Admin truy cập được.</p>
        </div>
      </div>

      <div className="flex bg-white rounded-lg border border-slate-200 p-1 shadow-sm w-fit max-w-full overflow-x-auto">
        {tabs.filter(t => !t.creatorOnly || isCreator).map(({ key, label, Icon }) => (
          <button key={key} onClick={() => setTab(key)}
            className={cn("flex items-center gap-2 px-5 py-2 text-sm font-semibold rounded-md transition-all whitespace-nowrap", tab === key ? "bg-amber-500 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50")}>
            <Icon className="w-4 h-4" />{label}
          </button>
        ))}
      </div>

      {tab === "monitor" && isCreator ? <DataHealthView />
        : tab === "api" ? <ApiTester />
        : tab === "db" ? <DbBrowser />
        : tab === "sql" ? <QueryStudio />
        : <DataLineageMap />}
