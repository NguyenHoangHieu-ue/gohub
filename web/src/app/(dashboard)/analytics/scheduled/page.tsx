  ...
  const renderForm = () => (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="space-y-2">
          ...
          <input type="text" placeholder="VD: Báo cáo doanh thu hằng ngày"
            className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <select className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 font-bold text-slate-700"
            ...
            {scheduleMode !== "custom" && (
              <input type="time" className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
                value={scheduleTime} onChange={e => setScheduleTime(e.target.value)} />
            )}
            {scheduleMode === "weekly" && (
              <select className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            ...
            {scheduleMode === "custom" && (
              <input type="text" placeholder="VD: 0 8 * * 1-5"
                className="w-full px-4 py-2 font-mono bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <div className="relative">
            ...
            <textarea rows={10} placeholder="Hướng dẫn cho AI Analyst..."
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none"
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
        ...
        <button onClick={handleSave} className="flex items-center gap-2 px-6 py-2 bg-brand-600 text-white rounded-xl text-sm font-bold hover:bg-brand-700 transition-colors shadow-sm">
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Clock className="w-6 h-6 text-brand-600" />Scheduled Messages</h1>
        ...
        {canEdit && !isAdding && !isEditing && (
          <div className="flex items-center gap-2">
            <button onClick={handleCreateWeeklyReport} disabled={generatingReport}
              className="flex items-center gap-2 px-4 py-2.5 bg-white text-brand-600 border border-brand-200 rounded-xl font-bold text-sm hover:bg-brand-50 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
              {generatingReport
                ? <div className="w-4 h-4 border-2 border-brand-400 border-t-transparent rounded-full animate-spin" />
                : <FileText className="w-4 h-4" />}
              {generatingReport ? "Đang tạo báo cáo..." : "Create Weekly Report"}
            </button>
            <button onClick={startAdding} className="flex items-center gap-2 px-4 py-2.5 bg-brand-600 text-white rounded-xl font-bold text-sm hover:bg-brand-700 transition-all shadow-sm">
      ...
      {!isAdding && !isEditing && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          {loading ? (
            <div className="flex justify-center p-12"><div className="w-8 h-8 border-4 border-brand-600 border-t-transparent rounded-full animate-spin" /></div>
          ) : messages.length === 0 ? (
            ...
            <div className="p-4">
              <DataTable
                ...
                columns={[
                  {
                    key: "status", label: "Trạng thái", render: msg => canEdit ? (
                      <button onClick={() => toggleActive(msg)}
                        className={`flex items-center gap-1.5 px-3 py-1 rounded-full font-bold transition-colors ${msg.is_active ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-200" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>
                        <Power className="w-3 h-3" />{msg.is_active ? "Active" : "Paused"}
                      </button>
                    ) : (
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-bold ${msg.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                        <Power className="w-3 h-3" />{msg.is_active ? "Active" : "Paused"}
                      </span>
                    ),
                  },
                  {
                    key: "name", label: "Tên & Lịch", render: msg => (
                      <div>
                        <div className="font-bold text-slate-800">{msg.name}</div>
                        <div className="flex items-center gap-1.5 font-mono text-slate-500 mt-1"><Clock className="w-3 h-3" />{msg.cron_expression}</div>
                      </div>
                    ),
                  },
                  { key: "prompt", label: "Prompt", render: msg => <div className="max-w-xs line-clamp-2 text-slate-600" title={msg.prompt}>{msg.prompt}</div> },
                  { key: "created_by", label: "Người tạo", render: msg => <div className="flex items-center gap-1.5 text-slate-600"><User className="w-3.5 h-3.5 text-slate-400" />{msg.created_by || "—"}</div> },
                  { key: "last_run", label: "Lần chạy cuối", render: msg => <span className="text-slate-500">{msg.last_run_at ? new Date(msg.last_run_at).toLocaleString("vi-VN") : "Chưa chạy"}</span> },
                  ...(canEdit ? [{
                    key: "actions", label: "Thao tác", align: "right" as const, render: (msg: ScheduledMessage) => (
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => startEditing(msg)} className="p-2 text-brand-600 hover:bg-brand-50 rounded-lg transition-colors" title="Sửa"><Edit2 className="w-4 h-4" /></button>
